custom_data_prep = function(input_path, output_path){
  ...
  REMOVE_PARAM_LIST = c("ACA_LINE_NUM", "ACTTR_CBORE_PSN", "ACTTR_HOLE_PSN", "ACTTR_POST_PSN", 
                        ...
                        "VCM_TOP_THREAD_PSN","LOT_NUM", "RECORD_SEQ", "PART_NUM.1")
  ...
  # QPM_LAT_DATA <- "QPM_DASHBOARD_VCMA.rda"
  ...
  cpk_plotting <- function(raw, trigger, com){
    ...
    sel_columns = names(raw)[names(raw) %in% c("SUPPLIER_NAME","PART_NUM", "SBR_NUM", "DATA_SOURCE_TYPE", "STKEVT_CELL_ID", "HEAD_PSN", "GRMLD_CELL_ID", "GRMLD_HEAD_PSN", "CAVITY_NUM", "MOLD_NUM", "GROUP_DATETIME", "GRP_YEAR_WEEK", "EVENT_DATE", xname)]
    index_columns = c("SUPPLIER_NAME", "PART_NUM", "Parameter", "CAVITY_NUM", "MOLD_NUM", "DATA_SOURCE_TYPE")
    ...
    for(i in 1:nrow(Index_plot)) {
      ...
      if(commodity == "HSA_MARPOSS") {
        ...
        X = Raw_Plot[SUPPLIER_NAME ==  Index_plot[i,SUPPLIER_NAME] & PART_NUM  ==  Index_plot[i,PART_NUM] & CAVITY_NUM == Index_plot[i,CAVITY_NUM] & MOLD_NUM == Index_plot[i,MOLD_NUM] & DATA_SOURCE_TYPE == Index_plot[i,DATA_SOURCE_TYPE]]
        ...
        if(is.na(Index_plot[i,]$CAVITY_NUM)) {
          X = Raw_Plot[SUPPLIER_NAME ==  Index_plot[i,SUPPLIER_NAME] & PART_NUM  ==  Index_plot[i,PART_NUM] & DATA_SOURCE_TYPE == Index_plot[i,DATA_SOURCE_TYPE]]
          setorderv(Raw_Plot,c("GROUP_DATETIME"),c(1))
        } else {
          X = Raw_Plot[SUPPLIER_NAME ==  Index_plot[i,SUPPLIER_NAME] & PART_NUM  ==  Index_plot[i,PART_NUM] & CAVITY_NUM == Index_plot[i,CAVITY_NUM] & DATA_SOURCE_TYPE == Index_plot[i,DATA_SOURCE_TYPE]]
          setorderv(X,c("GROUP_DATETIME"),c(1))
        }
        ...
        X = Raw_Plot[(SUPPLIER_NAME == Index_plot[i,SUPPLIER_NAME] & PART_NUM  ==  Index_plot[i,PART_NUM]) & DATA_SOURCE_TYPE == Index_plot[i,DATA_SOURCE_TYPE]]
        ifelse(commodity == "MOTOR", setorderv(X,c("GRP_YEAR_WEEK",xname),c(1,1)),setorderv(X,c("GRP_YEAR_WEEK","GROUP_DATETIME",xname),c(1,1,1)))
        print("OTHER")
  ...
  get_trigger_con <- function(working_data){
    ...
    if ("TRIGGER_LEVEL" %in% names(working_data)) {
      ...
      if(!unique(working_data$TRIGGER_LEVEL) %in% "OOC") {
        
        working_data <- left_join(working_data, trigger_mod, by = "TRIGGER_LEVEL")
        
        if ("Cpk" %in% names(working_data)) {
          
          working_data <- working_data %>% mutate(trigger_con = paste(ifelse(Cpk < Cpk_con & !is.na(Cpk_con) & N >= 15, paste0("CPK < ", Cpk_con), ""), ifelse(Diff > Diff_con & !is.na(Diff_con), paste0("Diff > ", Diff_con), ""), ifelse(Slope > Slope_con & !is.na(Slope_con), paste0("Slope > ", Slope_con), "")))
    ...
    if(all(!is.na(working_data$OOC))){
      ...
      if (!'trigger_con' %in% names(working_data)) {
        working_data$trigger_con <- ""
      }
      
      if (working_data$trigger_con == "  " | working_data$trigger_con == " ") {
        
        working_data$trigger_con <- gsub(" ", "", working_data$trigger_con, fixed = TRUE)
        working_data[sel_con,]$trigger_con <- paste0(working_data[sel_con,]$trigger_con, working_data[sel_con,]$OOC)
    ...
    sample_checker = working_data$Trigger %in% 'YES' & working_data$trigger_con == "  "
    
    if (any(sample_checker) == TRUE) {
      
      working_data[sample_checker,]$Trigger <- NA
      
      working_data <- working_data[complete.cases(working_data$Trigger),]
  ...
  # QPM_LAT_DATA = c("QPM_DASHBOARD_PCBA.rda") 
  # QPM_LAT_DATA = c("QPM_DASHBOARD_SPACER.rda","QPM_DASHBOARD_RAMP.rda") 
  # QPM_LAT_DATA = c("QPM_DASHBOARD_SDRAM.rda","QPM_DASHBOARD_SOC.rda") 
  
  # QPM_LAT_DATA = "QPM_DASHBOARD_RAMP.rda"
  
  write.csv(data.frame("SUPPLIER_NAME" = NA, "PART_NUM" = NA, "QPM_PARAMETER" = NA, "COMMODITY" = NA, "SUPPLIER_KEY" = NA), 
			file = file.path(ResultsPath, "E1087845", "SPEC_CHECKER_LAT.csv"),
			row.names = F)
    ...
    # tryCatch({
      ...
      if ("DATA_SOURCE_TYPE" %in% names(data_set) == FALSE) {
        ...
        if (any(grepl("DATA_SOURCE_TYPE", names(data_set)))) {
          names(data_set)[grepl("DATA_SOURCE_TYPE", names(data_set))] <- "DATA_SOURCE_TYPE"
        }
      ...
      } else if(commodity == 'WOVENTAPE') {
        names(data_set)[names(data_set) %in% "DATA_SORUCE_TYPE"] <- "DATA_SOURCE_TYPE"
      ...
      if (commodity %in% c("ACA", "ACT", "COIL", "HOOKUP") & class(trigger) != "logical") {
      ...
      } else if (class(trigger) == "logical") {
        print("Triggers is N/A")
      ...
      if (any(!is.na(trigger))) {
        ...
        selecter <- c("SUPPLIER_NAME","PART_NUM","Parameter","CAVITY_NUM","MOLD_NUM","GROUP_NUM","N","Mean","Std","Median","DATA_SOURCE_TYPE","ETL_LOAD_DATE","YEAR_WEEK","Target","USL","LSL","Cpk","p","Diff","Slope","OOC","Trigger","TRIGGER_LEVEL","trigger_con","Material_type","Product")
        ...
        col_sorter <- c("PART_NUM","Parameter","SUPPLIER_NAME","CAVITY_NUM","MOLD_NUM","Material_type","N","Mean","Std","Cpk","p","Diff","Slope","GROUP_NUM","ETL_LOAD_DATE","DATA_SOURCE_TYPE","YEAR_WEEK","Median","Target","USL","LSL","Product","OOC","Trigger","TRIGGER_LEVEL","trigger_con","DO_NUM","DO_QTY")
        
        trigger <- trigger[,order(match(names(trigger), col_sorter))]
        ...
        if (commodity == "DSP") {
          ...
          names(trigger) <- c("PART_NUM", "Parameter", "SUPPLIER_NAME", "CAVITY_NUM", "Material type", "N", "Mean", "Std", "Cpk", "p", "Diff", "Slope", "GROUP_NUM", "ETL_LOAD_DATE", "DATA_SOURCE_TYPE", "YEAR_WEEK", "Median", "Target", "USL", "LSL", "Product", "OOC", "Trigger", "TRIGGER_LEVEL", "Trigger Criteria", "DO_NUM", "DO_QTY")
          ...
          # trigger <- trigger[,c("PART_NUM", "Parameter", "SUPPLIER_NAME", "CAVITY_NUM", "MOLD_NUM", "N", "Mean","Std", "Cpk", "p", "Diff", "Slope", "GROUP_NUM", "ETL_LOAD_DATE", "DATA_SOURCE_TYPE", "YEAR_WEEK", "Median", "Target", "USL", "LSL", "Product", "OOC", "Trigger", "TRIGGER_LEVEL", "Trigger Criteria", "DO_NUM", "DO_QTY")]
          
		  names(trigger) <- c("PART_NUM", "Parameter", "SUPPLIER_NAME", "CAVITY_NUM", "MOLD_NUM", "N", "Mean","Std", "Cpk", "p", "Diff", "Slope", "GROUP_NUM", "ETL_LOAD_DATE", "DATA_SOURCE_TYPE", "YEAR_WEEK", "Median", "Target", "USL", "LSL", "Product", "OOC", "Trigger", "TRIGGER_LEVEL", "Trigger Criteria", "DO_NUM", "DO_QTY")
          ...
          names(trigger) <- c("PART_NUM", "Parameter", "SUPPLIER_NAME", "N", "Mean", "Std", "Cpk", "p", "Diff", "Slope", "GROUP_NUM", "ETL_LOAD_DATE", "DATA_SOURCE_TYPE", "YEAR_WEEK", "Median", "Target", "USL", "LSL", "Product", "OOC", "Trigger", "TRIGGER_LEVEL", "Trigger Criteria", "DO_NUM", "DO_QTY")
