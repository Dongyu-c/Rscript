custom_data_prep = function(input_path, output_path){
  ...
  process_list = c("CHI_SKYBOLT_ODT_RAW.rda")
  ...
  for(loop in 1:length(process_list)){
	...
	tryCatch({
		...
		chi_data <- chi_data[,unique(names(chi_data))]
  ...
  filter_process_list = c('CHI_CIMARRON_SUMMARY.rda', 'CHI_CIMARRONBP_SUMMARY.rda', 'CHI_SKYBOLT_SUMMARY.rda')
  
  filter_df = data.frame(product_internal_name = NA, run_type = NA, line_num = NA, dom = NA, product_name = NA)

  for(loop in 1:length(process_list)){
  
	chi_data <- readRDS(file.path(input_path, process_list[loop]))
	
	chi_data <- unique(chi_data[,c("product_internal_name", "run_type", "line_num", "dom", "product_name")])	
	
	filter_df <- rbind(chi_data, filter_df)
	
	rm(chi_data)
	
  }
  
  saveRDS(filter_df, file.path(input_path, "filter.rda"))
